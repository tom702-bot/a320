...
An installable, iPhone-friendly study app for A320 limitations, systems questions, memory items, fill-in practice, an interactive ECAM-style electrical synoptic and a provisional IAE V2500 engine-systems explorer.
...
The ECAM-style electrical page is a switchable system simulator: sources, batteries, transformer rectifiers, the static inverter and principal contactors can be selected on or isolated. It recalculates downstream bus power and explains each transfer, loss or load shed. Five presets remain available for normal, single-generator, APU, emergency-generator and battery-only study. Its architecture and procedural constraints come from the trainer's existing DSC-24 / QRH-referenced content and require confirmation against the current approved Ansett DSC and QRH.

The separate IAE V2500 engine page is an interactive systems study assistant. It traces engine architecture, FADEC/thrust control, fuel, oil, air/start and hydraulically powered thrust-reverser layouts. Each screen has selectable normal, degraded and failure modes plus tappable component explanations and applicable limitations. Engine start is retained as one part of the air/start system rather than the main activity. The module is explicitly provisional: its engine-specific logic and limits are paraphrased from the supplied IAE engineering appendix for A/C 20-IMHT / 19-IMHE, dated 2015-2018 and marked FOR ENGINEERING USE ONLY. It must be checked against a current approved IAE FCOM before operational use.
