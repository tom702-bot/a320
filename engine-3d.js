(function(){
  ...
  const PARTS={
    nacelle:{label:'NACELLE & CASES',short:'NACELLE',system:'architecture',center:[-1.0,1.8,0],
      ...
      operation:'CUTAWAY removes a section of the cases to expose the gas path. The cases shown are a simplified study representation, not a maintenance model.',facts:['BYPASS DUCT','ENGINE CASES','CONCEPTUAL GEOMETRY']},
    intake:{label:'INLET LIP & DIFFUSING INTAKE',short:'INLET',system:'architecture',center:[-4.48,1.45,.45],
      ...
      operation:'Inlet distortion, turbulence, icing or foreign-object damage can reduce compressor stability margin. P2 is sensed in the inlet region for EPR control.',facts:['CAPTURE + DIFFUSE','P2 SENSING','CONCEPTUAL GEOMETRY']},
    ...
    lpc:{label:'LOW-PRESSURE COMPRESSOR',short:'LP COMP',system:'architecture',center:[-2.35,.65,.38],
      ...
      operation:'The 2.5 bleed valve improves stall margin during starting, low power and transients. Its fail-safe position is open.',facts:['FAN + 4 STAGES','LP SHAFT','CONCEPTUAL GEOMETRY']},
    ...
    diffuser:{label:'COMPRESSOR DIFFUSER',short:'DIFFUSER',system:'architecture',center:[.72,.68,.42],
      ...
      operation:'Conceptual diffuser geometry only. The supplied FCOM does not establish a numeric diffuser velocity; no such value is taught or graded.',facts:['DIVERGENT DUCT','VELOCITY ↓','PRESSURE ↑']},
    combustor:{label:'ANNULAR COMBUSTION CHAMBER',short:'COMBUSTOR',system:'air',center:[1.25,.42,.42],
      ...
      operation:'The primary zone anchors the flame; dilution air produces a more uniform turbine-inlet temperature. Use the current ECAM and FCOM for start limits.',facts:['ANNULAR','PRIMARY + DILUTION','CONCEPTUAL GEOMETRY']},
    ...
    ngv:{label:'TURBINE NOZZLE GUIDE VANES',short:'TURBINE NGV',system:'architecture',center:[2.02,.64,.45],
      ...
      operation:'An NGV and following turbine wheel form a turbine stage. The vanes convert pressure energy into directed kinetic energy before the rotor.',facts:['STATOR ROW','PRESSURE → VELOCITY','CONCEPTUAL GEOMETRY']},
    ...
    shafts:{label:'CONCENTRIC SHAFTS',short:'SHAFTS',system:'architecture',center:[.3,.05,.25],
      ...
      operation:'Splitting the compressor into independently rotating spools helps match rear-compressor speed to airflow and improves stall margin.',facts:['TWIN SPOOL','N1 + N2','CONCEPTUAL GEOMETRY']},
    ...
    gearbox:{label:'ACCESSORY GEARBOX',short:'GEARBOX',system:'architecture',center:[-.7,-1.3,.35],
      ...
      operation:'Engine 1 drives the green hydraulic pump and Engine 2 the yellow hydraulic pump. Each engine-driven generator is a primary electrical source.',facts:['HP SHAFT DRIVE','MULTIPLE ACCESSORIES','CONCEPTUAL GEOMETRY']},
    ...
    sensors:{label:'ENGINE SENSORS & EGT HARNESS',short:'SENSORS',system:'instrumentation',center:[3.95,.98,.62],
      ...
      operation:'ECAM presents engine monitoring parameters. Sensor placement in this cutaway is illustrative; exact thermocouple and vibration-pickup locations are not established by the supplied FCOM.',facts:['P2 / P5','N1 / N2 / EGT','VIB + OIL + FF']},
