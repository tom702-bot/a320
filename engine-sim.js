(function(){
  ...
  const systems={
    ...
    instrumentation:{
      tab:'INDICATIONS / ECAM',ata:'DSC 70-95',title:'ENGINE SENSING & ECAM INDICATIONS',overviewId:'upper',
      ...
      nodes:[
        node('p2',25,45,120,68,'INLET P2',component('INLET PRESSURE P2','Measures pressure in the inlet region ahead of the fan.','P2 combines with exhaust pressure P5 to provide sensed EPR for normal V2500 thrust control.','Loss of valid P2 and/or P5 removes sensed EPR and causes rated N1 reversion.','DSC-70-35-20')),
        node('p5',25,145,120,68,'EXHAUST P5',component('EXHAUST PRESSURE P5','Measures pressure downstream of the last turbine stage.','P5 divided by P2 is the engine pressure ratio used as the normal thrust-setting parameter.','Loss of the P5 sensing path causes the same rated N1 reversion as loss of P2.','DSC-70-35-20')),
        node('n1',25,260,120,68,'N1 SPEED\nPICKUP',component('N1 SPEED SENSING','Measures fan and LP-spool rotational speed and sends an electrical speed signal for control and display.','N1 is displayed as a percentage and becomes the control parameter after EPR reversion.','A lost or invalid signal removes a key control and monitoring input; use current ECAM/QRH for the exact effect.','DSC-70-20')),
        node('n2',25,375,120,68,'N2 SPEED\nPICKUP',component('N2 SPEED SENSING','Measures HP-spool rotational speed for start sequencing, fuel scheduling and monitoring.','N2 is displayed as a percentage. FADEC uses it throughout start and running operation.','Invalid N2 removes a critical start/control input; the diagram does not substitute for current ECAM logic.','DSC-70-20/80')),
        node('egt',190,45,135,68,'EGT THERMO-\nCOUPLES',component('EGT THERMOCOUPLE HARNESS','A circumferential set of thermocouples senses gas temperature in the jet-pipe region downstream of the turbine stages.','The combined electrical signal provides EGT for primary indication and start/limit monitoring.','A high or rapidly rising EGT can indicate an abnormal start or compressor instability; use current ECAM/QRH for response.','')),
        node('ff',190,145,135,68,'FUEL FLOW\nTRANSMITTER',component('FUEL FLOW TRANSMITTER','Measures fuel flow in the delivery line to the combustion fuel nozzles.','Its electrical signal provides instantaneous fuel-flow indication and supports fuel-used computation.','A failed signal affects the indication, while an actual fuel-flow interruption affects combustion through the separate fuel system.','')),
        node('vib',190,260,135,68,'N1 / N2 / TURB\nVIB SENSORS',component('ENGINE VIBRATION SENSORS','Sense vibration in the compressor spools and turbine region as an engine-health parameter.','Electrical signals are displayed with the secondary engine information.','High vibration can accompany compressor damage or stall and requires current ECAM/QRH assessment.','')),
        node('oil',190,375,135,78,'OIL P / T / Q\n+ FILTER',component('OIL SYSTEM SENSORS','Measure delivery pressure, temperature downstream of the fuel/oil heat exchanger, tank quantity and filter restriction.','The data is supplied for secondary engine indication and alerting.','An out-of-range parameter or clog input produces the applicable indication; limits and crew action come from current approved data.','DSC-70-50')),
        ...
        node('upper',755,60,120,135,'UPPER ECAM\nEPR · N1 · EGT',component('PRIMARY ENGINE INDICATION','Presents the principal thrust and temperature parameters in the normal crew scan.','The presentation identifies EPR, N1 and EGT as primary engine information on the upper screen.','Out-of-tolerance data changes the display and can be accompanied by the applicable alert; use current ECAM/QRH.','')),
        node('lower',755,315,120,145,'ENGINE SD\nOIL · FF · VIB',component('SECONDARY ENGINE INDICATION','Presents detailed engine-system information when the engine page is displayed.','The presentation groups oil pressure/temperature, fuel flow and vibration with the secondary information.','Abnormal values aid diagnosis but do not by themselves replace the ECAM procedure.',''))
      ],
      edges:[
      ...
      modes:[
        ...
        mode('highvib','HIGH VIBRATION','caution','ENGINE VIBRATION HIGH','One or more vibration channels report an abnormal level in the displayed engine vibration parameters.',[
    ...
    stability:{
      tab:'COMPRESSOR / STALL',ata:'DSC 70-20/65',title:'COMPRESSOR STABILITY & STALL',overviewId:'vsv',
      ...
      nodes:[
        ...
        node('indication',720,300,145,120,'INDICATIONS\nBANG · EGT · N1/N2\nVIB · THRUST',component('COMPRESSOR STALL INDICATIONS','Combines the presentation signs that may accompany compressor stall or surge.','Expected stable operation produces smooth parameters, normal vibration and proportional thrust response.','Possible signs include abnormal noises/bangs, inlet or exhaust flame, fluctuating EGT/N1/N2, vibration, sluggish thrust response and high or rapidly rising EGT.','')),
        node('thrust',745,480,120,60,'NET THRUST',component('AVAILABLE ENGINE THRUST','Represents the usable result of stable fan bypass and core operation.','With both spools stable, thrust responds predictably to the command.','A compressor stall reduces airflow and can cause a substantial or fluctuating loss of thrust.',''))
      ],
      edges:[
  ...
  // FCOM-backed indication content replaces unsupported sensor-location assertions.
  const byNode=(system,id)=>systems[system].nodes.find(n=>n.id===id);
  Object.assign(byNode('instrumentation','egt').detail,{purpose:'Provides exhaust-gas-temperature indication for engine monitoring.',normal:'EGT is a primary parameter on the upper E/WD. Exact thermocouple placement is not established by the supplied FCOM.',ref:'DSC-70-95 · IAE A/C 19-IMHE, 12 AUG 2015 · PDF pp.2315–2318'});
  Object.assign(byNode('instrumentation','vib').detail,{purpose:'Provides the engine vibration parameter shown on the ENG and CRUISE SD pages.',normal:'The displayed vibration values assist monitoring; exact pickup locations are not specified here.',ref:'DSC-70-95 · IAE A/C 19-IMHE, 12 AUG 2015 · PDF p.2322'});
  byNode('instrumentation','vib').label='ENGINE VIBRATION';
  Object.assign(byNode('instrumentation','oil').detail,{purpose:'Provides oil pressure, temperature, tank quantity and main-scavenge-filter clog information.',normal:'The IAE schematic shows pressure sensing after the fuel/oil exchanger and temperature sensing in the scavenge return path.',ref:'DSC-70-50 · PDF pp.2288–2289'});
  Object.assign(byNode('instrumentation','ff').detail,{purpose:'Provides the fuel-flow parameter displayed on the upper E/WD.',normal:'Fuel used is computed by the DMC and displayed separately on the ENG SD page.',ref:'DSC-70-95 · IAE A/C 19-IMHE, 12 AUG 2015 · PDF pp.2318, 2321'});
  Object.assign(byNode('instrumentation','upper').detail,{normal:'Primary engine parameters include EPR, EGT, N1, N2 and fuel flow.',ref:'DSC-70-95 · IAE A/C 19-IMHE, 12 AUG 2015 · PDF pp.2315–2318'});
  byNode('instrumentation','upper').label='UPPER E/WD\nEPR · EGT · N1\nN2 · FUEL FLOW';
  Object.assign(byNode('instrumentation','lower').detail,{normal:'Secondary parameters include fuel USED, oil quantity/pressure/temperature and vibration, with start and clog indications as applicable.',ref:'DSC-70-95 · IAE A/C 19-IMHE, 12 AUG 2015 · PDF pp.2321–2322'});
  byNode('instrumentation','lower').label='ENGINE SD\nFUEL USED\nOIL · VIB';
  byNode('oil','filter').detail.abnormal='A supply-filter restriction affects the pressure path. The displayed OIL CLOG indication is associated with the main scavenge filter, not an assumed separate supply-filter warning.';
  const oilPressure=systems.oil.edges.find(e=>e.id==='ind1');oilPressure.from='fuelhx';oilPressure.route=[];
  const oilTemp=systems.oil.edges.find(e=>e.id==='ind2');oilTemp.from='scavfilter';oilTemp.route=[];
  // Generic cutaway mechanics are explanatory. No physical simulation or maintenance diagnosis.
  const sourcePages={architecture:'2260–2263, 2270–2273',fadec:'2264–2269, 2272–2279',fuel:'2282–2287',oil:'2288–2289, 3737',air:'2290–2291, 2296–2309, 3738',instrumentation:'2272–2273, 2314–2322',stability:'2267, 2284–2285, 2291; stall indications 2788',reverser:'2292–2295, 3738'};
  Object.entries(systems).forEach(([id,s])=>{s.sourcePages=sourcePages[id];s.nodes.forEach(n=>{if(!n.detail.ref)n.detail.ref=id==='stability'?'General engine explanation; FCOM DSC-70-20/45/65 and PRO-ABN-ENG indications':'FCOM DSC-70-95';});});
  systems.stability.modes.find(m=>m.id==='recovery').text='Illustrative reduced-thrust state only. Use the applicable engine-stall procedure; the supplied general stall text has distinct ground and flight branches (PDF p.2788). This diagram is not a recovery checklist.';
  ...
  function renderInfo(){
    ...
    nameEl.textContent=detail.name;purposeEl.textContent=detail.purpose;normalEl.textContent=detail.normal;abnormalEl.textContent=detail.abnormal;refEl.textContent='SOURCE: '+detail.ref+' · FCOM PDF pp.'+system.sourcePages+' · Training explanation; component-failure propagation and geometry are illustrative, not verified operational logic.';
