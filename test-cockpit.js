...
for(const name of ['cockpit-native.js?v=49','cockpit-native.css?v=49','cockpit-systems.js?v=49','flow-procedures.js?v=49','cockpit-view.js?v=49','cockpit-view.css?v=49','flow-evidence.js?v=49'])assert(html.includes(name)&&sw.includes(name),name+' offline inclusion');
...
assert.equal(flow.CONTROL_DEFS.filter(c=>c.photoMapped).length,400);assert(flow.CONTROL_DEFS.filter(c=>!c.photoMapped).every(c=>c.panel==='checks'));assert(html.includes('cockpit-photo-layout.js?v=49')&&sw.includes('cockpit-photo-layout.js?v=49'));assert(sw.includes('./fbw-cockpit.png'));
