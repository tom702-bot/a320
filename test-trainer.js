...
async function offlineTests(){
  ...
  assert.match(await flows.text(),/flow-sim\.js\?v=48/);
  ...
  assert.equal(await (await caches.open('a320-trainer-v48')).match('missing.js'),undefined,'404 responses are not cached');
